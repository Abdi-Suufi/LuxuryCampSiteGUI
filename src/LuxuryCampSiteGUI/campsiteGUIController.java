...
public class campsiteGUIController implements Initializable {
    ...
    public boolean firstNameValidation() {
        ...
        //goes through each character to see if it isnt a letter then alerts if thats the case
    ...
    public boolean telephoneValidation() {
        ...
        // alert if phonenumber is less or hgiher than 11 numbers
        (telephoneTextField.getText().length() > 11 || telephoneTextField.getText().length() < 11) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Error");
            alert.setHeaderText("Invalid Telephone Number");
            alert.setContentText("Telephone number must be 11 digits long");
            alert.showAndWait();
            return false; // Validation fails
        }
        return true; // Validation is successful
    ...
    void checkedIn(ActionEvent event) {
        ...
        // validation on telephone number
        if (!telephoneValidation()){
            return;
        }

        // validation on number of guests
        ...
        //  validation on number of nights
        if (!numberOfNightsValidation()) {
            return;
        }

        //  validation on if accommodation is available or not
